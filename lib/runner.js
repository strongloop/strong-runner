...
var debugIo = process.env.STRONGRUNNER_DEBUG_IO;
var fs = require('fs');
...
Runner.prototype._restart = function _restart(status) {
  ...
  // XXX(sam) though a fast stop followed by a start can confuse this heuristic,
  // to make it more accurate we would need a better state machine to track the
  // current state of the app, and it's not so clear the criteria for liveness.
  // In practice, it's only wrong in unit tests, and the message is only a
  // diagnostic.
  ...
  var self = this;
  setTimeout(function() {
    // Between the unexpected exit and this delay, an explicit stop may have
    // occurred, in which case, we should not be restarting.
    if (!self.stopped)
      self.start();
  }, 1000);
};
